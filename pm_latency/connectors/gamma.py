...
def fetch_markets_by_condition_ids(
    ...
    *,
    ...
    """GET /markets?condition_ids=0x...&condition_ids=0x... (Gamma accepts repeated param)."""
    if not condition_ids:
        return []
    root = base_url.rstrip("/")
    parts = "&".join(f"condition_ids={quote(cid)}" for cid in condition_ids)
    url = f"{root}/markets?{parts}"
    ctx = ssl.create_default_context()
    req = urllib.request.Request(
        url,
        headers={
